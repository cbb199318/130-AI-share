...
> 使用方式：
> 将以下任意一条 **UI Style Prompt** 直接追加到你的 **App 首页结构 Prompt** 末尾即可。
>
> 推荐格式：
> ```
> [App Home Structure Prompt]
>
> Apply the following UI style:
> [UI Style Prompt]
> ```

---

## 01. Minimal iOS Style（极简 iOS 风格）
```text
Apply a minimal iOS-style interface.
Use generous white space, soft shadows, rounded cards, and subtle dividers.
Typography should feel clean and system-native.
```

## 02. Apple Fitness Style（Apple 健身风格）
```text
Apply an Apple Fitness–inspired UI.
Use bold metrics, rounded cards, vibrant accent colors, and clean typography.
Focus on clarity, motion, and energy.
```

## 03. Material Design (Google)（谷歌 Material 风格）
```text
Apply a Material Design UI style.
Use clear elevation, consistent spacing, and structured cards.
Follow Google-style components and layout logic.
```

## 04. Modern Dark Mode（现代暗黑模式）
```text
Apply a modern dark mode UI.
Use deep dark backgrounds, soft contrast, and subtle highlights.
Ensure readability and a premium feel.
```

## 05. Cyberpunk / Futuristic（赛博朋克 / 未来科技风）
```text
Apply a futuristic cyberpunk UI style.
Use dark backgrounds, neon accents, glowing elements, and high contrast.
Create a tech-driven, sci-fi atmosphere.
```

## 06. Glassmorphism（玻璃拟态风格）
```text
Apply a glassmorphism UI style.
Use translucent cards, background blur, soft borders, and light reflections.
Maintain a clean and airy visual feel.
```

## 07. Neumorphism (Soft UI)（新拟态 / 软 UI）
```text
Apply a neumorphism UI style.
Use soft shadows, subtle highlights, and pill-shaped components.
Create a tactile and gentle interface.
```

## 08. Brutalist UI（粗犷主义 / 野性 UI）
```text
Apply a brutalist UI style.
Use strong contrast, bold typography, and raw layouts.
Prioritize function over decoration.
```

## 09. Luxury / Premium（高端奢华风格）
```text
Apply a luxury premium UI style.
Use elegant typography, muted colors, large spacing, and refined details.
Create a calm and high-end experience.
```

## 10. Startup SaaS Style（创业公司 / SaaS 产品风）
```text
Apply a modern startup SaaS UI style.
Use clean cards, clear CTAs, friendly colors, and a professional layout.
Focus on usability and clarity.
```

## 11. AI Product Style（AI 产品风格）
```text
Apply an AI-powered product UI style.
Use futuristic accents, gradient highlights, and clean layouts.
Create a smart and intelligent visual tone.
```

## 12. Fitness Energy Style（高能量健身风）
```text
Apply a high-energy fitness UI style.
Use bold colors, strong contrast, and dynamic visual elements.
